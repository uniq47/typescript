...
// sothe value one will have T as their variable and theri Value will be U as their variable and thene it should return thr array where the first value will be T and second value will be U
function outputArray<T, U>(valueOne: T, valueTwo: U):[T,U]{// it is going to accept two values and return an array of two values so we can use generic type here
    return [valueOne, valueTwo];
}

outputArray(1,"string");

// function getArray(items){
//     return new Array().concat(items);
// }

// const outputNumber = getArray([1,2,3]);
// console.log(outputNumber);

// const outputString = getArray(["string", "string2"]);
// console.log(outputString);

// outputNumber.push("Nepal"); 
// we changed it frrom function getArray(items)  to function getArray<T>(items: T):T[] function getArray<T>(items: T) so we can pass any type of data in this function
function getArray<T>(items: T):T[]// retrun type is array of T
{
    //
    return new Array<T>().concat(items);
}

const outputNumber = getArray<number []>([1,2,3]);
console.log(outputNumber);

const outputString = getArray<string []>(["string", "string2"]);
console.log(outputString);
...
// convert string to nun = Number(value);
 //or +"value"


//console.log((<number>value).toFixed(2)); di not use
